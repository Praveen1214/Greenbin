    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (response.data.status === "Login Success") {
          ...
          Alert.alert("Registarion Failed", "Something went wrong. Please try again.");
        }
...
className = {`flex-row items-center justify-center p-4 rounded-full ${form.gender === value
...
color = { form.gender === value ? "white" : "black" }
  ...
  <Text
        className={
  `ml-2 font-semibold ${form.gender === value ? "text-white" : "text-black"
  }`
}
      >
  { value }
